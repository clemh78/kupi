...
angular.module('services', [])
    ...
    .factory('serviceUser', function($http, $rootScope) {
        return {
            ...
            register : function(login, pass, display_name, code, email) {
                return $http({
                    method: 'POST',
                    url: 'api/users',
                    headers: { 'Content-Type' : 'application/x-www-form-urlencoded' },
                    data: $.param({"login" : login, "password" : pass, 'display_name' : display_name, 'room_code' : code, 'email' : email})
                });
            },
    ...
    .factory('serviceGame', function($http) {
        return {
            ...
            GetNext : function(token){
                return $http.get('api/games?token=' + token + '&status!=completed&team1_id!=null&team2_id!=null&orderby=date&order=ASC');
            },
            GetPrevious : function(token){
                return $http.get('api/games?token=' + token + '&status=completed&orderby=date&order=DESC');
            },
